...
export function DockLayout({
  ...
  const style: CSSProperties = {
    gridTemplateColumns: `${layout.left ? layout.leftWidth : 0}px 1fr ${layout.right ? layout.rightWidth : 0}px`,
    gridTemplateRows: `${layout.top ? layout.topHeight : 0}px 1fr ${layout.bottom ? layout.bottomHeight : 0}px`,
  }

  const hide = (id: PanelId) => setLayout((p) => ({ ...p, [id]: false }))

  return (
    <div className="hud-grid" style={style}>
      <div style={{ gridColumn: '1 / 4', gridRow: 1, minHeight: 0, overflow: 'hidden' }}>
        {layout.top ? <div className="panel panel-top" style={{ height: '100%' }}>{top}</div> : null}
      </div>
